...
menu_top = menu(u'Main Menu', [
	...
	urwid.Divider(),
	urwid.AttrMap(E(urwid.CheckBox, ["Using Strengthener"], '#2', {"help":"Whether or not the Strengthener processes should be used, particularly if enabled, there will be allocated on strengthener process per CDCL process, which will be used to strengthen (ie. reduce the number of literals) in the conflict clauses of the CDCL procedure."}), None, focus_map='reversed'),
	urwid.Divider(),
	urwid.Text("SLS Helpers:"),
	urwid.AttrMap(E(urwid.CheckBox, ["Using gNovelty+ Helpers"], '!kdlswra #1', {"help":"Whether or not to use the gNovelty+ SLS helper processes, particuarly a number of SLS processes will be allocated per CDCL process, and will help the CDCL by (a) finding solutions and (b) offering variable suggestions for the CDCL to select."}), None, focus_map='reversed'),
	sub_menu(u'gNovelty+ Configuration', [
		urwid.AttrMap(E(urwid.IntEdit, [u'gNovelty+ per CDCL: ',1], 'k', {"help":"the number of gNovelty+ SLS helpers per CDCL to attempt to allocate"}), None, focus_map='reversed'),
		urwid.Divider(),
		urwid.AttrMap(E(urwid.IntEdit, [u'gNovelty+ decision interval: ',30], 'd', {"help":"The gNovelty+ SLS helpers will be allocated to work at varying depths in the CDCL search, the decision interval is the desired spacing between the gNovelty+ allocated workers in the CDCL search tree, a spacing too large will result in the SLS process being allocated too sparsely, and thus they become less efficient, a spacing too small, and they become entirely localised to the most recent decisios in the CDCL process, and then become entirely inefficient. appropriate decision interval is some reasonable proportion (say 40%) of the expected search depth of the CDCL processes"}), None, focus_map='reversed'),
		urwid.AttrMap(E(urwid.IntEdit, [u'gNovelty+ solution checking time: ',30] ,'l', {"help":"The CDCL process checks the gNovelty+ SLS helpers for solutions every so many decisions, the more frequently the check is performed, the less likely that the SLS processes will have a backlog of solutions to pass up, however the more inhibited the CDCL is in its own search, if there are expected to be many solutions then decreasing the checking time may be a good idea, however if there are few solutions, then lengthening the solution checking time may be desirable"}), None, focus_map='reversed'),
		urwid.AttrMap(E(urwid.IntEdit, [u'gNovelty+ suggestion size: ',30], 's', {"help":"The size of the buffer that the SLS processes will report to the CDCL process, the greater the size of the buffer, the more comprehensive the SLS processes will report their trace back to the CDCL to help it follow the same path as the SLS. setting this value too high, will overload the CDCL with variable suggestions that are quckly changing and superfluous to the search, setting this value too low will mitigate the effectiveness of SLS suggestions as only the most static variables will be reported back to the CDCL - which may not even be particularly meaningfull. Broadly, the suggestion size should be comparable to the SLS decision interval."}), None, focus_map='reversed'),
		urwid.AttrMap(E(urwid.CheckBox, ["gNovelty+ clause weights "], 'w', {"help":"This option sets the SLS to dynamically try to weigh the satisfaction of particular clauses as its search proceeds, particularly it increases the weight of satisfying persistantly unsatisfied clauses. This is logical for enabling the SLS to escape local minima and find solutions to SAT queries more generally, however it potentially comes at the cost of being a good suggestion mechanism for the CDCL, as it attempts to escape local minima on its own accord rather than reporting suggestions of local minia to the CDCL for it to elminiate. This option is disabled by default, but should be considered if the SAT problem is expected to be friendly to SLS finding solutions over the CDCL process"}), None, focus_map='reversed'),
		urwid.Divider(),
		urwid.Text("CDCL heuristic rotation scheme:"),
		urwid.AttrMap(E(urwid.RadioButton, [sls_heuristic,u'slsfirst'], '@rslsfirst', {"help":"The CDCL has a choice of how to select variables, particularly if it should choose suggestions from the SLS over variable selections to satisfy the most recently added learnt clauses (and thereafter defaulting to VSIDS heuristic), this option instructs CDCL to prioritise variable suggestions from SLS, and should be used where SLS suggestions are tuned and expected to be of quality."}), None, focus_map='reversed'),
		urwid.AttrMap(E(urwid.RadioButton, [sls_heuristic,u'cdclfirst'], '@rcdclfirst', {"help":"The CDCL has a choice of how to select variables, particularly if it should choose suggestions from the SLS over variable selections to satisfy the most recently added learnt clauses (and thereafter defaulting to VSIDS heuristic), this option instructs CDCL to prioritise variable selections to satisfy recently learned clauses over variable suggestions from SLS."}), None, focus_map='reversed'),
		urwid.Divider(),
		urwid.Text("SLS suggestion heuristic:"),
		urwid.AttrMap(E(urwid.RadioButton, [sls_heuristic_rotation_scheme,u'most recent flip (ie `ghosts` mode)'], '@aghosts', {"help":"The SLS has options on how to report variable suggestions to the CDCL, particularly if it should report a collated and ranked list of variables ordered according to when they were last flipped, or simply a list of the variables that it most recently flipped (including duplicates), the latter is 'ghost' mode, as duplicate variables suggestions are consequently ignored by the CDCL, whereas the former is non-'ghosts' mode, where flipped variable values have more persistance in the SLS variable suggestions."}), None, focus_map='reversed'),
		urwid.AttrMap(E(urwid.RadioButton, [sls_heuristic_rotation_scheme,u'most relevent flip (non `ghosts` mode)'], '@anonghosts', {"help":"The SLS has options on how to report variable suggestions to the CDCL, particularly if it should report a collated and ranked list of variables ordered according to when they were last flipped, or simply a list of the variables that it most recently flipped (including duplicates), the latter is 'ghost' mode, as duplicate variables suggestions are consequently ignored by the CDCL, whereas the former is non-'ghosts' mode, where flipped variable values have more persistance in the SLS variable suggestions."}), None, focus_map='reversed'),
		urwid.Divider(),
		menu_button(u'Back', back_callback),
	]),
	urwid.Divider(),
	urwid.Text("Solutions Handler:"),
	urwid.AttrMap(E(urwid.RadioButton, [bdd_mode, u'Using Table Solutions Handler'], '@g0', {"help":"The Solutions Handler stores solutions between nodes as work to seed further computation, and there are different ways of storing these solutions, the Table Solutions Handler is the most simple, storing all the values of all the solutions as-is as a series of integer literals. this uncompressed and simple storage mechanims is uncomplicated and fast in simple cases where there are few messages between DAG nodes, however it will struggle with bloat trying to resolve messages together with DAGs with many solutions between nodes (and particularly with joins), the more expensive BDD Solutions Handler is designed to compress and represent/resolve solutions on a logical level using Binary Decision Diagrams."}), None, focus_map='reversed'),
	urwid.AttrMap(E(urwid.RadioButton, [bdd_mode, "Using Dumb Table Solutions Handler"], '@g2', {"help":"Similar to the Table Solutions Handler, except is will refuse to resolve incompatable messages where there is a join in the DAG, this mode will produce an error on any problem where resolving solutions between DAG nodes is required (which is presumably most problems). Hence is only suitable for particular problems where messages between nodes are always cross compatable. This mitigates resolving overhead in these cases."}), None, focus_map='reversed'),
	urwid.AttrMap(E(urwid.RadioButton, [bdd_mode, "Using BDD Solutions Handler"], '@g1' {"help":"The BDD Solutions Handler, stores messages/solutions between nodes on the problem DAG, it representes these solutions in their logical form using Binary Decision Diagrams, this compresses the messages in memory and minimises the number of subsequent runs, however it also has the potential to slow down the Master process in processing and compressing the messages that it handles. This BDD Solutions Handler is most appropriate where DAG nodes generate more solutions and there is need to logically resolve them together to minimise the volume of consequent processing required."}), None, focus_map='reversed'),
	sub_menu(u'BDD handler Configuration', [
		urwid.AttrMap(E(urwid.RadioButton, [bdd_config,u'- BDD paths encoding'], '@cpaths', {"help":"The BDD Solutions Handler (if enabled) needs to be instructed how to represent information that it subequently delivers to CDCL SAT workers, the 'paths' encoding is the simplest, encoding all branches in a BDD as separate clauses to be delivered to the CDCL to instruct it what to do/where not to go, etc. This contrasts with cubes and minisat encoding schemes which communicate this information more compactly at the cost of introducing additional variables"}), None, focus_map='reversed'),
		urwid.AttrMap(E(urwid.RadioButton, [bdd_config,u'- BDD cubes encoding'], '@ccubes', {"help":""}), None, focus_map='reversed'), #TODO
