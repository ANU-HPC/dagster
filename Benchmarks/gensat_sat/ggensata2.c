...
/**************************************************************/
{
 ...
 /* G2 = germe end               */
   fprintf(stderr,
	   ...
	   " 6. A = the number of variables passed from the first problem to the second \n" \
	   " 7. B = the proportion of clauses not in the first problem \n" \
 ...
 G1= atoi(argv[4]);
 G2= atoi(argv[5]);
 int A = atoi(argv[6]); // default 5
 double B = atof(argv[7]); // default 0.06
 ...
 for (k=G1; k<G2; k++) {
    ...
    // generate a random assignment as a solution we are going to ensure is SAT
    irandom(G);
    RANDMAX=2;
    int* solution = (int*)calloc(sizeof(int),N);
    for (int i=0; i<N; i++)
        solution[i] = private_random()*2-1;
    ...
    for (i=0;i<P;i++) {
      ...
      // detect if the clause makes the solution infeasible
      bool unsat_clause = true;
      for (int j=0; j<R; j++) {
          if (SAT[i][j] == abs(SAT[i][j])*solution[abs(SAT[i][j])-1]) {
              unsat_clause = false;
              break;
          }
      }
      // if clause makes the solution infeasible, then flip a variable randomly
      if (unsat_clause) {
        SAT[i][private_random()%R] *= -1;
      }
    }
