...
'''def dual_bound_or(term_dict1,lower1,upper1, term_dict2,lower2,upper2, aux_name1):
	# t1 >= upper1 - x(upper1-lower1)
	# t2 >= upper2 - (1-x)(upper2-lower2)
	#    >= lower2 + x(upper2-lower2)
	...
	for k2 in range(k1+1,letters): #letter 2
			...
			for xx in range(letter_width): # x-frequency xx
				for yy in range(letter_height): # y-frequency yy
					transformed_xx = min(xx,letter_width-xx)
					transformed_yy = min(yy,letter_height-yy)
					if transformed_xx**2+transformed_yy**2> (min(letter_width,letter_height)/7)**2:
						continue
					#if transformed_xx+transformed_yy> min(letter_width,letter_height)/7:
					#	continue
					#harshity = 10.0*((abs(xx+yy-letter_width/2-letter_height/2))**2)/((letter_width/2+letter_height/2)**2)
...
print(abs(fft2(letter_arrays[1]) - fft2(letter_arrays[0])))
