...
This copy is slightly different from the other ramsay generator, in that there is marginally different colour cardinality symmetry breaking.
whereas previously the colour enumeration symmetry was broken on the ordering of colours on the first vertex, in this version the colour cardinality is broken by an ordering on the absolute number of colours present in the graph. (eg. red is always the colour with occurs most often, green second often etc....)
