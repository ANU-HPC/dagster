...
def cbmc_join(input1, input2, output_cnf, output_dag):
	...
	for line in AI_lines1:
		...
		for i in range(2,len(split_line)):
			split_line_i = split_line[i].strip()
			if split_line_i.isdigit():
				cnf_vars.append(int(split_line_i))
	...
	for k in cnf_map1.keys():
		...
		for i in range(len(cnf_map1[k])):
			...
			cnf_map1_var = abs(cnf_map1[k][i])
			link = (cnf_map1_var,cnf_map2_var)
			if link not in AI_links:
				AI_links.append(link)
	# insert links
	AI_lines = 0
	for l in AI_links:
		output_cnf.write("{} {} 0\n".format(-l[0],l[1])) #A->B = -A\/B = -B->-A
		output_cnf.write("{} {} 0\n".format(l[0],-l[1])) #B->A = A\/-B = -A->-B
		AI_lines += 2
	...
	cnf_map1_variables = set()
	for k in cnf_map1.keys():
		cnf_map1_variables.update(set(cnf_map1[k]))
	output_dag.write("DAG-FILE\nNODES:2\nGRAPH:\n0->1:{}\nCLAUSES:\n0:0-{}\n1:{}-{}\nREPORTING:\n{}-{}\n".format(contract_sequence(list(cnf_map1_variables)), cnf_lines1-1,cnf_lines1,cnf_lines1+cnf_lines2+AI_lines-1,max_variables1+1,max_variables1+max_variables2))
