...
class Problem(object):
	...
	# NOTE; modification to the origional for proper superset counting.....
	def write_determinant_non_increasing_constraint(self):
		...
		for row in tqdm(range(self.n)):
			va = "i_{}_{}".format(row,row)
			va_end = "{}_{}".format(va,self.bit_resolution-1)
			va_pos = "{}_pos".format(va)
			self.isPositive(va,va_pos,nodes)
			va_pos_array.append(va_pos)
			va_neg_array.append(va_end)
		for row in tqdm(range(self.n)):
			for col in range(self.n):
				if col>row:
					continue # symmetry makes this moot
				v1 = "m_{}_{}".format(row,col)
				v2 = "i_{}_{}".format(row,col)
				v2_end = "{}_{}".format(v2,self.bit_resolution-1)
				v2_pos = "{}_pos".format(v2)
				self.isPositive(v2,v2_pos,nodes)
				
				va_pos = self.CNF_variables[va_pos_array[row]]
				va_neg = self.CNF_variables[va_neg_array[row]]
				vb_pos = self.CNF_variables[va_pos_array[col]]
				vb_neg = self.CNF_variables[va_neg_array[col]]
				
				different_sign = self.CNF_variables["different_sign_{}_{}".format(row,col)]
				self.add_clause([-va_pos,-vb_neg,-different_sign],nodes)
				self.add_clause([-va_pos,vb_neg,different_sign],nodes)
				self.add_clause([va_pos,-vb_neg,different_sign],nodes)
				self.add_clause([va_pos,vb_neg,-different_sign],nodes)
				
				self.add_clause([-self.CNF_variables[v2_pos], -different_sign,  self.CNF_variables[v1] ],nodes)
				self.add_clause([-self.CNF_variables[v2_end],  different_sign, -self.CNF_variables[v1] ],nodes)
