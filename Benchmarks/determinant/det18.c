...
int gjinv() {
	...
	for (k = 0; k < N; k++) {  /* Main loop */
		...
		if (A[k+k*N]<0) {
			for (j = k; j < N; j++) A[j+k*N] = -A[j+k*N];
			for (j = 0; j < N; j++) B[j+k*N] = -B[j+k*N];
		}
...
int run(long l) {
	...
	if (singular==0) {
		...
		for (int x=0; x<N; x++)
			for (int y=0; y<N; y++)
				if ( ((C[y*N+x]==0) && (B[y*N+x] > 0))   // if we can only increasing the bit and such an increase would increase the determinant, then fail
				  || ((C[y*N+x]==1) && (B[y*N+x] < 0)) ) // if we can only decrease the bit, and such a decrease would cause an increase in determinant, then fail
					return 1;
		return 2;
	}
	return 1;
...
int main() {
	long l = nondet_long();
	int r = run(l);
	__CPROVER_assert(r==1, "Determinant Assert");
}
