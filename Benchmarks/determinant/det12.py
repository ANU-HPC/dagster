...
class Problem(object):
	...
	def __init__(self,n,bit_resolution):
		...
		self.node_parts = defaultdict(self.new_variable_index)
	...
	def TerminatingAdder(self,var1,var2,var3,output):
		...
		self.add_clause([-var1,-var2,output]) # add no overflow constraint, negative + negative, -> negative result
		self.add_clause([var1,var2,-output]) # add no overflow constraint, positive + positive, -> positive result
...
##				self.add_comment("Creating GATED ROW")
				for k in range(self.n): # k is iterator
					...
					self.Gated("i_{}_{}".format(k,col),"m_{}_{}".format(row,k),"gi_{}_{}_{}".format(k,row,col))
				# perform the sum along the index
				self.AssertIsEqual("gi_{}_{}_{}".format(0,row,col),"gis_{}_{}_{}".format(0,row,col))
				for k in range(1,self.n):
					self.Add("gis_{}_{}_{}".format(k-1,row,col),"gi_{}_{}_{}".format(k,row,col),"gis_{}_{}_{}".format(k,row,col))
				# assert the result of the sum is zero if off-axis
				if col!=row:
					self.AssertIsZero("gis_{}_{}_{}".format(self.n-1,row,col))
				elif col==0: #else assert positive for first diagonal element
					self.AssertIsPositive("gis_{}_{}_{}".format(self.n-1,row,col))
				else: #else assert all diagonal elements are equal
					self.AssertIsEqual("gis_{}_{}_{}".format(self.n-1,row,col),"gis_{}_{}_{}".format(self.n-1,row-1,col-1))
	...
	def write_matrix_no_columns_equal(self):
		...
		for row1 in tqdm(range(self.n-1)):
			for row2 in range(row1+1,self.n):
				...
				self.Compare(v1_,v2_,g,l,g,l)
				self.add_clause([self.CNF_variables[g], self.CNF_variables[l]])
	...
	def write_matrix_no_zero_columns(self):
		...
		for col in tqdm(range(self.n)):
			...
			self.add_clause([-self.CNF_variables[t] for t in is_zeros])
