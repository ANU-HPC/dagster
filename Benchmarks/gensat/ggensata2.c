...
/**************************************************************/
{
 ...
 for (k=G1; k<G2; k++) {
    ...
    fprintf(f,"p cnf %d %d\n",N,P + 16);
    for ( int i = 1; i <= 8; i++)fprintf(f,"%d 0\n",i);
    for ( int i = 1; i <= 8; i++)fprintf(f,"%d 0\n",-i);
    for (i=0;i<P;i++) {
    ...
    for (int node = 256 * 0; node < 256; node++){ 
      fprintf(f,"%d->%d:",node,node+256);
      for (int j = 0; j < 8; j++){
	  fprintf(f,"%d",j+1);
	  if(j==8-1)fprintf(f,"\n");
	  else fprintf(f,",");
    ...
    for (int node = 256 * 1; node < 256 * 2; node++){
      fprintf(f,"%d->%d:",node,node+256);
      for (int j = 0; j < 12; j++){
	  fprintf(f,"%d",j+1);
	  if(j==12-1)fprintf(f,"\n");
    ...
    for (int node = 256*2; node < 256 * 3; node++){
      fprintf(f,"%d->%d:",node,256 * 3);
      for (int j = 0; j < N; j++){
	  fprintf(f,"%d",j+1);
	  if(j==N-1)fprintf(f,"\n");
	  else fprintf(f,",");
    ...
    for (; node < 256*2; node++){
      fprintf(f,"%d:%d-%d\n",node,16,P+16-1-(int)ceil(0.2*(double)P));//weak problem
      
    }
    for (; node < 256*3; node++){
      fprintf(f,"%d:%d-%d\n",node,16,P+16-1);//concrete problem
      
    }
    fprintf(f,"%d:%d-%d\n",256*3,16,P+16-1-(int)ceil(0.06*(double)P));
    fprintf(f,"REPORTING:\n");
    fprintf(f,"1-%d\n",N);
