...
header = '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" height="{0}" width="{0}" style="font-size: medium;">\n'.format(width)
tail = '</svg>'




cnf = [[1,2,3,4],[5,-4],[9,9,1,9,9,2],[1],[4,3]]
instructions = [1,-3,2,4,-7,5,None,-5]


cnf = [[1, 2, 3],[3, 4, 5],[5, 6],[-3, -5],[-2, 3, -4],[-4, 5, -6],[1, 2, 4],[3, 4, -6]]
instructions = [[1],[2],[3],[4],[5],[None],[-5],[6],[None],[-6],[None],[None],[None],[-4],[5],[None],[-5],[6],[float('inf')]]
instructions = [[1],[2],[3],[-5,6,-4],[float('inf')]]


cnf = [[1, 2, 3],[3, 4, 5],[5, 6],[-3, -5],[-2, 3, -4],[-4, 5, -6],[1, 2, 4],[3, 4, -6],[-2,-3,4,-6]]
instructions = [[1],[2],[3],[-5,6],[4,None,-4,None]]
instructions = [[-1],[2],[3],[-5,6],[4,None,-4,None]]


cnf = [[1, 2, 3],[3, 4, 5],[5, 6],[-3, -5],[-2, 3, -4],[-4, 5, -6],[1, 2, 4],[3, 4, -6],[-2,-3,4,-6],[-2,-3]]
instructions = [[1],[2],[-3,-4,-6,5],[float('inf')]]

output_svg = open("svg_out.svg","w")
...
def draw_tree(cnf, offset, instructions, style1, style2, circle_width, transform):
	variables = max([max([abs(aa) for aa in a]) for a in cnf])
	coordinates = [np.matrix([[0],[0]])]
	var_assignments = [[None]*variables]
...
def draw_tree2(cnf, offset, overall_instructions, style1, style2, circle_width, transform):
	variables = max([max([abs(aa) for aa in a]) for a in cnf])
	var_assignments = [[None]*variables]
	write_assignments_2(list(range(1,variables+1)),width-variables*30,30,30,defaultdict(lambda :"fill:none; stroke:black; stroke-width:1px"))
	write_cnf(cnf,0,0,100,500, var_assignments[-1])
	instructions = []
	for i, instruction_group in enumerate(overall_instructions):
		output_svg.write("<g id='part_{}'>".format(i))
		output_svg.write('<rect width="{0}" height="{0}" style="fill:rgb(253,246,227)" />'.format(width)) #fdf6e3
		write_assignments_2(list(range(1,variables+1)),width-variables*30,30,30,defaultdict(lambda :"fill:none; stroke:black; stroke-width:1px"))
		for instruction in instruction_group:
			...
			print(instructions)
		draw_tree(cnf, offset, instructions, style1, style2, circle_width, transform)
		output_svg.write("</g>")
